...
// (Helper functions extractRequestUrl, rewriteCodexUrl, readRequestBody, 
// createCodexHeaders, mapUsageLimit404, readErrorPreview omitted for brevity 
...
function rewriteCodexUrl(url: string): string {
  if (url.includes(CODEX_CONFIG.API_PATH)) return url;
...
async function readRequestBody(body: BodyInit): Promise<string> {
  ...
  if (typeof (body as Blob).text === "function") return await (body as Blob).text();
...
async function mapUsageLimit404(response: Response): Promise<Response | null> {
  ...
  if (!/usage_limit_reached|usage_not_included|rate_limit_exceeded|usage limit/i.test(haystack)) return null;
  const headers = new Headers(response.headers);
  return new Response(response.body, { status: 429, statusText: "Too Many Requests", headers });
}
...
function createCodexFetch(userId: string): typeof fetch {
  return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    const url = extractRequestUrl(input);
    if (!url.includes("/responses")) return fetch(input, init);

    const tokenValid = await ensureValidCodexToken(userId);
    if (!tokenValid) throw new Error("Codex authentication required");

    const accessToken = await getCodexAccessToken(userId);
    ...
    if (init?.body) {
      ...
      updatedInit = { ...init, body: JSON.stringify(transformed) };
    }
    ...
    for (let attempt = 0; ; attempt += 1) {
      ...
      try {
        response = await fetch(rewriteCodexUrl(url), { ...updatedInit, method: updatedInit?.method || "POST", headers });
      } catch (error) {
        const classification = classifyRecoverability({ provider: "codex", error, message: String(error) });
        if (!shouldRetry({ classification, attempt, maxAttempts: CODEX_MAX_RETRY_ATTEMPTS, aborted: init?.signal?.aborted ?? false })) throw error;
      ...
      if (!effectiveResponse.ok) {
        ...
        const classification = classifyRecoverability({ provider: "codex", statusCode: effectiveResponse.status, message: errorText });
        if (shouldRetry({ classification, attempt, maxAttempts: CODEX_MAX_RETRY_ATTEMPTS, aborted: init?.signal?.aborted ?? false })) {
          await sleepWithAbort(getBackoffDelayMs(attempt), init?.signal ?? undefined);
          continue;
        }
...
export async function getCodexProvider(userId: string): Promise<(modelId: string) => LanguageModel> {
  const openai = createOpenAI({
    name: "codex",
    baseURL: CODEX_CONFIG.API_BASE_URL,
    apiKey: DUMMY_API_KEY,
    fetch: createCodexFetch(userId),
  });
...
export function createCodexProvider() {
  throw new Error("createCodexProvider is deprecated. Use getCodexProvider(userId) instead.");
}
